...
3. **Data Cleaning** – Used Power Query to remove duplicates, handle nulls, fix data types, create age groups.
4. **Data Modeling** – Structured tables/relationships (star schema).
5. **DAX Measures** – Built calculations for Attrition Rate, Averages, Counts.
6. **Dashboard Design** – Planned layout: KPI cards, charts, slicers, navigation panel.
7. **Visualization** – Created donut, bar, pie, matrix, and line charts for attrition analysis.
8. **Interactivity** – Added slicers (Age Group) and department filter buttons with cross-filtering.
9. **Testing** – Validated KPI accuracy and slicer functionality.
10. **Formatting** – Applied consistent theme, colors, and layout polish.
11. **Insights** – Derived findings (e.g., high attrition in Operations, early-career employees).
